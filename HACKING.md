...
- **`srv/`** — Service layer (stateful, side-effectful):
  - `srv/gtk/` — All GTK UI code. Package alias is `gtksrv`. Entry point is `gtksrv.Run()` which creates the Adwaita app and main window. The `Window` struct owns navigation, clipboard state, and wires up all page callbacks.
